...
class App {
  ...
  constructor(opts) {
    ...
    this.router.get('/flatten', (req, res) => this.flatten(req, res));
  ...
  flatten(req, res) {
    const data = App.getCommonAttributes(req, 'flatten');
    data.pagetitle = 'Flattening message';
    data.exampleR = fs.readFileSync('./var/example-clientregistration.txt', 'utf8');
    data.exampleJWKS = fs.readFileSync('./var/example-jwks.json', 'utf8');

    res.render('flatten', data);
  }
