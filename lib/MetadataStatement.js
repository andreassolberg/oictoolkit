...
class MetadataStatement {
  ...
  decode(keystore) {
    ...
    if (!payload.metadata_statements) {
      this.log.info(payload, 'payload.metadata_statements was not set');
      return [this];
    }

    const x = new MetadataStatementEncoded(payload.metadata_statements[0]);
    this.log.info(x.toJSON(), 'decoding this');
    return x.decode(keystore);

    // return Promise.all(
    //   payload.metadata_statements.map(
    //     encoded => new MetadataStatementEncoded(encoded)
    //   )
    //   .map(st => st.decode(keystore))
    //   .map(msp => msp.then(ms => mslist.push(_.concat(this, ms))))
    // );
  }
