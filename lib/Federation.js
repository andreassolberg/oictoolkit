...
class Federation {
  ...
  decode(jwksraw, msraw) {
    let ms;
    return jose.JWK.asKey(jwksraw[0])
      .then(key => MetadataStatement.decode(key, msraw))
      .then((x) => {
        console.log('----- MS ----');
        console.log(x);
        ms = x;
  ...
  getMS() {
    const msFeide = new MetadataStatement('https://feide.no/', {
      iss: 'https://edugain.org/',
      id_token_signing_alg_values_supported: ['RS256', 'RS512'],
      claims: ['sub', 'name', 'email', 'picture'],
    }, this.keystore);

    const msKalmar = new MetadataStatement('https://kalmar.org/', {
    ...
    msEntity.addMetadataStatement(msFeide);
    msEntity.addMetadataStatement(msKalmar);
